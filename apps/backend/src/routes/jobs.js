...
const COL = 'jobs';
const STATUS = ['queued', 'in_progress', 'waiting_parts', 'completed', 'cancelled'];
...
router.get('/', async (req, res, next) => {
  try {
    ...
    if (req.query.mechanicId) q = q.where('mechanicId', '==', req.query.mechanicId);
    if (req.query.customerId) q = q.where('customerId', '==', req.query.customerId);
...
router.post('/', async (req, res, next) => {
  try {
    const allowed = ['title', 'customerId', 'mechanicId', 'start', 'end', 'status', 'notes'];
    ...
    if (!(await exists('customers', data.customerId))) return next(httpError(400, 'invalid customerId'));
    if (data.mechanicId && !(await exists('mechanics', data.mechanicId))) return next(httpError(400, 'invalid mechanicId'));
    const now = Date.now();
...
router.patch('/:id', async (req, res, next) => {
  try {
    ...
    if (patch.status && !isEnum(patch.status, STATUS)) return next(httpError(400, 'invalid status'));
    if (patch.customerId && !(await exists('customers', patch.customerId))) return next(httpError(400, 'invalid customerId'));
    if (patch.mechanicId && !(await exists('mechanics', patch.mechanicId))) return next(httpError(400, 'invalid mechanicId'));
...
router.post('/:id/status', async (req, res, next) => {
  try {
    ...
    if (!isEnum(status, STATUS)) return next(httpError(400, 'invalid status'));
    const ref = db.collection(COL).doc(req.params.id);
    ...
    try {
      await db.collection('status_logs').add({ jobId: ref.id, status, ts });
    } catch (_) { /* ignore */ }
